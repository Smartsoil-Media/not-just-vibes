...
import { localComplete } from '../ai/local.js';
import { SIDE_QUEST_SYSTEM, SKILL_DETECT_SYSTEM } from '../ai/prompts.js';
import { tryParseJson } from '../ai/postprocess.js';

export const skillsRouter = new Hono();

skillsRouter.get('/catalog', (c) => c.json(skills));

skillsRouter.get('/state/:projectId', async (c) => {
...
skillsRouter.post('/side-quest', async (c) => {
...
skillsRouter.get('/side-quests/:projectId', async (c) => {
  const rows = await db
    .select()
