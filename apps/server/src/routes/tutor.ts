...
import { localComplete, localStream } from '../ai/local.js';
import { cloudComplete, cloudStream } from '../ai/cloud.js';
import {
  BREAKDOWN_SYSTEM,
...
tutor.post('/stuck', async (c) => {
  ...
  await persistMessage(parsed.data.projectId, 'user', null, `[I'm stuck L${level}] ${parsed.data.question ?? ''}`);

  const iter =
    source === 'local'
      ? localStream(system, userMsg)
      : cloudStream([{ text: system, cache: true }], userMsg, { maxTokens: 1024 });
...
tutor.post('/inline-hint', async (c) => {
  const parsed = inlineHintRequest.safeParse(await c.req.json());
  if (!parsed.success) return c.json({ error: parsed.error.flatten() }, 400);
  const before = parsed.data.code.slice(Math.max(0, parsed.data.cursor - 400), parsed.data.cursor);
...
tutor.post('/commit-review', async (c) => {
...
tutor.get('/messages/:projectId', async (c) => {
